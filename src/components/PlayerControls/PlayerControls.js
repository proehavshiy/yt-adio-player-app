...
import {
  nextTrack, prevTrack, nextTrackInSequence, switchChangingTrackMode, switchLoopTrackMode, setVolume,
...
function PlayerControls({
  ...
  // const [volume, setVolume1] = useState(setVolume);
  ...
  const toggleVolume = () => {
    ...
    if (volume < 1) dispatch(setVolume(1));
  };

  return (
    <div className={cn(controls)}>
      <div className={cn(sideControls)}>
        <button className={cn(volumeBtn, { volumeBtnDisabled: volume === 0 })}
